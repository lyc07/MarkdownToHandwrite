...
from .handwriting import HandwritingEngine, wrap_text
...
class ReportRenderer:
    ...
    def render(self, blocks: list[Block]) -> list[Image.Image]:
        for index, block in enumerate(blocks):
            if isinstance(block, HeadingBlock):
                ...
                if index + 1 < len(blocks) and isinstance(blocks[index + 1], FormulaBlock):
                    formula = self._render_formula_image(blocks[index + 1].text, seed_extra=f"lookahead:{index + 1}")
                    required = self._paragraph_height(text) + self._formula_height(formula)
                    if required <= self.bottom_limit - self.margin_top:
                        self._ensure_space(required)
                    self.formula_cache[index + 1] = formula
                if any(part.kind == "math" for part in block.parts):
                    self._draw_rich_paragraph(block.parts)
                else:
                    self._draw_paragraph(text)
            elif isinstance(block, FormulaBlock):
                self._draw_formula(block.text, image=self.formula_cache.pop(index, None))
    ...
    def _resolve_background_path(self) -> Path | None:
        ...
        if not self.config.background.auto_discover:
            return None
        checked: set[Path] = set()
        for directory in (Path.cwd() / "background", PROJECT_ROOT / "background"):
    ...
    def _draw_heading(self, block: HeadingBlock) -> None:
        ...
        if self.config.layout.number_sections and level <= 3:
            text = f"{self._next_section_number(level)} {text}"
        font_pt = {
        ...
        lines = wrap_text(self.engine, text, size, self.content_w)
        total_h = gap_before + len(lines) * line_h + gap_after
        self._ensure_space(total_h)
        self.y += gap_before
        align = "center" if level == 1 else "left"
        for line in lines:
            self._paste_text(line, self.margin_left, self.y, size, self.content_w, align=align)
            self.y += line_h
        self.y += gap_after
    ...
    def _draw_paragraph(self, text: str) -> None:
        ...
        for paragraph in text.split("\n"):
            ...
            for line_index, line in enumerate(lines):
                ...
                self._paste_text(line, x, self.y, size, width)
                self.y += line_h
            first = False
        self.y += gap

    def _paragraph_height(self, text: str) -> int:
        if not text:
            return 0
        size = pt_to_px(self.config.handwriting.body_font_pt, self.dpi)
        line_h = round(size * self.config.handwriting.line_spacing)
        ...
        for paragraph in text.split("\n"):
            if not paragraph.strip():
                total += gap
                continue
            width = self.content_w - indent if first else self.content_w
            total += len(wrap_text(self.engine, paragraph.strip(), size, width)) * line_h
            first = False
        return total + gap
    ...
    def _draw_rich_paragraph(self, parts: list[InlinePart]) -> None:
        ...
        gap = mm_to_px(self.config.layout.paragraph_gap_mm, self.dpi)
        indent = round(size * self.config.layout.first_line_indent_em)
        lines: list[tuple[bool, list[tuple[str, object, int]]]] = []
        current: list[tuple[str, object, int]] = []
        used = 0
        first = True

        def current_width() -> int:
            return self.content_w - (indent if first else 0)

        def flush() -> None:
            nonlocal current, used, first
            while current and current[-1][0] == "text" and not str(current[-1][1]).strip():
                current.pop()
            if current:
                lines.append((first, current))
                first = False
            current = []
            used = 0
            ...
            if part.kind == "break":
                ...
                try:
                    box = self.formula_renderer.render_inline(
                        ...
                        seed_extra=f"inline:{len(self.pages)}:{self.y}:{part_index}",
                    )
                except LatexRenderError as error:
                    self._report_formula_error(part.text, error)
                    box = self.formula_renderer.blank_inline(part.text, size, self.content_w - indent)
        ...
        for is_first, line in lines:
            ...
            def flush_text() -> None:
                ...
                image = self.engine.render_line(
                    ...
                    size,
                    width,
                    seed_extra=f"rich:{len(self.pages)}:{self.y}:{visual_index}",
                )
            ...
            self._ensure_space(line_h)
            cursor_x = x
            for image, item_baseline in visuals:
                self.page.alpha_composite(image, (round(cursor_x), self.y + baseline - item_baseline))
    ...
    def _draw_list(self, block: ListBlock) -> None:
        ...
        for offset, item in enumerate(block.items):
            ...
            text = parts_to_text(item)
            prefix_w = round(self.engine.measure(prefix, size))
            lines = wrap_text(self.engine, text, size, self.content_w - indent - prefix_w)
            for index, line in enumerate(lines or [""]):
                self._ensure_space(line_h)
                if index == 0:
                    self._paste_text(prefix + line, self.margin_left + indent // 2, self.y, size, self.content_w - indent // 2)
                else:
                    self._paste_text(line, self.margin_left + indent, self.y, size, self.content_w - indent)
                self.y += line_h
        self.y += gap
    ...
    def _draw_table(self, block: TableBlock) -> None:
        ...
        header_height = self._row_height(block.headers, widths, size, line_h, pad) if block.headers else 0
        if block.headers:
            self._ensure_space(header_height + line_h)
            self._draw_table_row(block.headers, widths, header_height, size, line_h, pad, header=True)
        for row in block.rows:
            row_height = self._row_height(row, widths, size, line_h, pad)
            if self.y + row_height > self.bottom_limit:
                self._new_page()
                if block.headers:
                    self._draw_table_row(block.headers, widths, header_height, size, line_h, pad, header=True)
            self._draw_table_row(row, widths, row_height, size, line_h, pad, header=False)
    ...
    def _row_height(self, row: list[list], widths: list[int], size: int, line_h: int, pad: int) -> int:
        max_lines = 1
        for index, width in enumerate(widths):
            cell = row[index] if index < len(row) else []
            max_lines = max(max_lines, len(wrap_text(self.engine, parts_to_text(cell), size, max(10, width - 2 * pad))))
        return max_lines * line_h + 2 * pad

    def _draw_table_row(self, row: list[list], widths: list[int], height: int, size: int, line_h: int, pad: int, header: bool) -> None:
        x_positions = [self.margin_left]
        for width in widths:
            x_positions.append(x_positions[-1] + width)
        y0 = self.y
        y1 = self.y + height
        ink = color(self.config.handwriting.ink_color, 145 if header else 115)
        ...
        for index, width in enumerate(widths):
            text = parts_to_text(row[index]) if index < len(row) else ""
            lines = wrap_text(self.engine, text, size, max(10, width - 2 * pad))
            y = y0 + pad
            for line in lines:
                self._paste_text(line, x_positions[index] + pad, y, size, width - 2 * pad)
                y += line_h
        self.y = y1
    ...
    def _rough_line(self, x0: int, y0: int, x1: int, y1: int, fill: tuple[int, ...], width: int = 1) -> None:
        ...
        for step in range(segments + 1):
            t = step / segments
            x = x0 + (x1 - x0) * t + self.rand.gauss(0, 0.7)
            y = y0 + (y1 - y0) * t + self.rand.gauss(0, 0.7)
